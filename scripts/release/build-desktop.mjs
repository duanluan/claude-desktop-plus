...
async function normalizeBundleArtifactNames(targets) {
  ...
  for (const target of buildTargets) {
    await normalizeMacUpdaterArtifact(target);
  }
}

async function normalizeMacUpdaterArtifact(target) {
  ...
  const destination = path.join(macosDir, `${appName}_${arch}.app.tar.gz`);
